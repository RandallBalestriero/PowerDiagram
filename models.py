...
class DNNClassifier(object):
	def __init__(self,input_shape,model_class,lr=0.0001,optimizer = tf.train.AdamOptimizer,ALPHA=0,train_option='none'):
		...
		with tf.device('/device:GPU:0'):
			...
			self.all_reconstruction_loss = [l.reconstruction_loss for l in self.layers[1:]]
			self.reconstruction_loss = ALPHA*tf.add_n(self.all_reconstruction_loss)/(len(self.layers)-1)
			if(train_option == 'during' or train_option == 'both'):
				self.training_loss = self.crossentropy_loss+self.reconstruction_loss
			else:
	...
	def _prefit(self,X,y,indices,update_time=50):
		...
		if(self.train_option=='both' or self.train_option=='pre'):
			for II in xrange(n_epochs/4):
				pretrain_loss.append(self._fit(X,y,indices))
		...
		if(self.train_opt=='pretrain' or self.train_opt=='both'):
	...
	def __init__(self,input_shape,model_class,lr=0.0001,optimizer = tf.train.AdamOptimizer):
		...
		with tf.device('/device:GPU:0'):
			...
			count_number_of_params()
        	        ...
        	        self.variables     = tf.trainable_variables()
			self.gamma_set     = tf.assign(self.layers[1].gamma,self.layers[1].gamma*2)
                        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
                        with tf.control_dependencies(update_ops):
...
class SpecialDense:
        ...
        def get_layers(self,input_variable,input_shape,training):
                layers = [InputLayer(input_shape,input_variable)]
                layers.append(SpecialDenseLayer(layers[-1],16,constraint=self.constraint))
                layers.append(SpecialDenseLayer(layers[-1],32,constraint=self.constraint))
                layers.append(DenseLayer(layers[-1],self.n_classes,nonlinearity=None,training=training,first=True))
		self.layers = layers
                return self.layers
